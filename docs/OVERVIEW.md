...
1. **Claim**: the supervisor picks a ready bead, moves it from `open` to
...
4. **RESULT.json**: before exiting, the worker writes
   ...
   `blocked`.
5. **Reap**: once the subprocess exits, the supervisor's reap step reads
...
|---|---|---|---|
| Coder subprocess is running but silent for a long time | Every status tick (`orchestrator/stall.py`) compares "now" to the last write in the attempt's `events.jsonl` | Past the warning threshold the task is logged as stalled; if the configured action is `kill`, the supervisor kills the coder subprocess | `stall_warning_minutes` (15), `stall_action` (kill) |
...
| A blocked bead is left alone | The supervisor's triage loop (`orchestrator/triage.py`) runs on a schedule and asks one non-blocking question per blocked bead with a proposed fix | An operator answers via the UI or the `ask_human` integration; fleet applies the answer (retry, retry with a bigger model, close, or ignore for a while) on the next tick | `triage_interval_minutes` (15; 0 disables) |
