...
3. **Worker steps**: the worker prepares its prompt (optionally compacting
   a long history first), then the `LlmSession` step
   (`workers/llm_session.py`, built on the `workers/session/` package)
   runs the coder as a subprocess and streams its events (tool calls,
   tokens used, errors) while refreshing a heartbeat lease. Small
   ordered monitors watch the session: one tracks token usage, one
   tracks elapsed time, one probes coder health, one keeps the lease
   alive (`workers/session/monitors.py`).
4. **RESULT.json**: before exiting, the worker writes `RESULT.json` at the
   task root with a status of `done`, `partial`, or `blocked`.
...
poking around will find: `task.json` (title, status, lease info — owned
by `state/task_meta.py`), `attempts.jsonl` (one line per attempt, start
and end — owned by `state/attempt_journal.py`), an `attempts/<n>/`
folder per attempt holding that attempt's raw logs, its `run.json`
lease/launch record (owned by `state/run_file.py` as `RunRecord`) and a
snapshot of its result, plus the worker's own working files at the task
root (`STATE.md`, `RESULT.json`, `outputs/` — owned by
...
`runtime.toml` file can override it) or a fixed constant in
`src/fleet/core/limits.py`. Retry/streak counting itself lives in the
`RETRY_TABLE` data table in `src/fleet/core/retry_policy.py`, which reads
the attempt history through `state/attempt_journal.py`.
...
| Attempt runs longer than allowed | The `AttemptBudget` monitor (`workers/session/monitors.py`) tracks elapsed time against a budget | The coder subprocess is sent SIGTERM (then SIGKILL if needed) and the attempt ends with outcome `KILLED`, reason `timeout` | `max_attempt_minutes` (120) |
| Repeated stalls/timeouts on the same task | The `RETRY_TABLE` in `core/retry_policy.py` counts consecutive `KILLED` outcomes with reason `stalled`/`timeout` as a "stall" streak | Each one is released for a fresh retry; once the streak reaches `STALL_MAX_ROUNDS` (a fixed constant, currently 2) the bead is blocked for human review | `stall_warning_minutes` (15) governs detection; the block threshold is the code constant `STALL_MAX_ROUNDS` (2) |
| Worker's context window fills up | The `ContextGauge` monitor (`workers/session/monitors.py`) tracks token usage against the coder's context limit | Past the checkpoint percentage a checkpoint marker is written so the next attempt compacts; past the kill percentage the subprocess is killed with outcome `CONTEXT_PRESSURE`, and the next attempt resumes with compaction | `context_checkpoint_pct` (75), `context_kill_pct` (90) |
| Model API rate-limits the coder | The coder reports a `rate_limit` event, or a health probe (`HealthProbe` in `workers/session/monitors.py`) detects silence consistent with a provider error during a quiet period | The subprocess is stopped with outcome `RATE_LIMIT`; the attempt is released (not counted against any retry limit) and retried after a wait | outcome is `rate_limit`; probe timing uses `core/limits.py` `PROBE_SILENCE_SEC` (60) and `RATE_LIMIT_PROBE_SILENCE_SEC` (300) |
...
- `fleet task <id> log` prints the latest attempt's log; `state` and
  `result` print `STATE.md` and `RESULT.json` instead of the log.
