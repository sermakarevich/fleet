"""The one rule for merging bead status into a task's on-disk metadata.
...
Lives in ``core`` (no I/O, pure dicts) so both ``state`` (GC) and
``beads`` (reconcile) can import it without breaking the layer rule
