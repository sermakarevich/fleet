...
function statusChip(status: string): ChipStyle {
  ...
  return { label: statusLabel(status || '?'), color: fg, bg };
}
  ...
  return (
    <div style={styles.drawerOverlay} onClick={onClose}>
      <aside style={styles.drawer} onClick={e => e.stopPropagation()}>
        ...
        {error && <p style={{ ...styles.msg, color: T.colors.danger }}>Error: {String(error)}</p>}
...
export function BeadsPage() {
  ...
  if (error) {
    return <p style={{ ...styles.msg, color: T.colors.danger }}>Error: {String(error)}</p>;
  }
