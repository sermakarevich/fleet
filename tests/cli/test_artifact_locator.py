...
def test_locate_attempt_files_use_latest_start(tmp_path: Path) -> None:
    ...
    fleet_home = _home_with_attempts(tmp_path, "t-1")
    base = fleet_home / "tasks" / "t-1" / "attempts" / "2"
    assert locate(fleet_home, "t-1", "log") == base / "log.jsonl"
    assert locate(fleet_home, "t-1", "events") == base / "events.jsonl"
    assert locate(fleet_home, "t-1", "stderr") == base / "log.stderr"
    assert locate(fleet_home, "t-1", "prompt") == base / "prompt.md"


def test_locate_explicit_attempt(tmp_path: Path) -> None:
    """An explicit attempt number wins over the latest start."""
    fleet_home = _home_with_attempts(tmp_path, "t-1")
    assert locate(fleet_home, "t-1", "events", attempt=1) == (
        fleet_home / "tasks" / "t-1" / "attempts" / "1" / "events.jsonl"
    )
...
def test_locate_state_is_task_level(tmp_path: Path) -> None:
    ...
    fleet_home = _home_with_attempts(tmp_path, "t-1")
    assert locate(fleet_home, "t-1", "state") == fleet_home / "tasks" / "t-1" / "STATE.md"


def test_locate_result_prefers_live_file(tmp_path: Path) -> None:
    """Live RESULT.json wins when present."""
    fleet_home = _home_with_attempts(tmp_path, "t-1")
    live = fleet_home / "tasks" / "t-1" / "RESULT.json"
    live.write_text("{}")
    assert locate(fleet_home, "t-1", "result") == live
...
def test_locate_result_falls_back_to_snapshot_then_legacy(tmp_path: Path) -> None:
    ...
    fleet_home = _home_with_attempts(tmp_path, "t-1")
    snapshot = fleet_home / "tasks" / "t-1" / "attempts" / "2" / "RESULT.json"
    snapshot.write_text("{}")
