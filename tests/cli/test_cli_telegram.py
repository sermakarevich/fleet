...
def test_telegram_status_full_token_never_in_stdout(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("fleet.integrations.telegram.setup.get_me", return_value={"username": "mybot"}),
    ):
        result = runner.invoke(app, ["telegram", "status"], env={"TELEGRAM_BOT_TOKEN": token})
...
def test_telegram_status_invalid_token_exits_nonzero(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch("fleet.integrations.telegram.setup.get_me", side_effect=RuntimeError("Unauthorized")),
    ):
        result = runner.invoke(app, ["telegram", "status"], env={"TELEGRAM_BOT_TOKEN": "bad:token"})
    assert result.exit_code != 0
...
def test_telegram_status_fully_configured_exits_zero(tmp_path: Path) -> None:
    ...
    with (
        ...
        result = runner.invoke(app, ["telegram", "status"], env={"TELEGRAM_BOT_TOKEN": "123:tok"})
    assert result.exit_code == 0
...
def test_telegram_test_success_path(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch("fleet.integrations.telegram.setup.send_message_raise") as mock_send,
    ):
        result = runner.invoke(app, ["telegram", "test"], env={"TELEGRAM_BOT_TOKEN": "123:tok"})
...
def test_telegram_setup_writes_chat_id(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch("fleet.integrations.telegram.setup.get_me", return_value={"username": "mybot"}),
        patch("fleet.integrations.telegram.setup.send_message_raise"),
    ):
...
def test_telegram_setup_token_validated_via_get_me(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch(
            "fleet.integrations.telegram.setup.get_me",
...
def test_telegram_setup_chat_discovery_persists_selection(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch("fleet.integrations.telegram.setup.get_me", return_value={"username": "mybot"}),
        patch("fleet.integrations.telegram.setup.get_updates", return_value=updates),
        patch("fleet.integrations.telegram.setup.send_message_raise"),
    ):
        result = runner.invoke(
            app,
...
def test_telegram_setup_chat_discovery_no_messages_exits_nonzero(tmp_path: Path) -> None:
    ...
    with (
        _patch_root(tmp_path),
        patch("fleet.integrations.telegram.setup.get_me", return_value={"username": "mybot"}),
        patch("fleet.integrations.telegram.setup.get_updates", return_value=[]),
    ):
        result = runner.invoke(
            app,
            ["telegram", "setup", "--yes"],
