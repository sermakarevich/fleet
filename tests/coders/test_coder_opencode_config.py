...
def test_build_argv_prompt_follows_the_isolated_worktree(tmp_path: Path) -> None:
    """Regression (fleet-o5xr2): the prompt pointed at the original repo while the
    process ran in the worktree, so resumed attempts edited the shared tree.
    opencode >= 2.0 has no `run --dir`; the cwd is the subprocess cwd."""
...
def test_build_argv_no_dir_flag_when_not_isolated(tmp_path: Path) -> None:
