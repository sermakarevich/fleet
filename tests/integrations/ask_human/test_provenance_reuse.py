...
def test_find_pending_ignores_resolved(tmp_path: Path):
    ...
    s.answer(qid, "papers/Talkio", answered_by="op")
...
def test_retry_reuses_pending_question_creates_no_new_row(tmp_path: Path):
    ...
    first = s.create(
        ...
        options=["papers/Talkio", "investment/2026-09-22-Talkio"],
        task_id="fleet-sa6ku",
        context=SOURCE,
    )
    ...
    async def scenario():
        async def operator():
            ...
            assert s.answer(first, "papers/Talkio", answered_by="op")
    ...
    assert result["answer"] == "papers/Talkio"
