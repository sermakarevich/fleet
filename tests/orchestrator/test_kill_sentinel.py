...
from fleet.orchestrator.kill_sentinel import kill_sentinel_tick, make_kill_sentinel
...
def test_tick_deletes_kill_file_and_kills_once(tmp_path: Path) -> None:
    ...
    asyncio.run(kill_sentinel_tick(st))
...
def test_default_interval_is_one_second() -> None:
    """Default cadence is 1.0s; factory arg overrides it."""
    assert make_kill_sentinel().interval_sec == 1.0
    assert make_kill_sentinel(interval_sec=0.01).interval_sec == 0.01
