"""Tests for locating HELPER_REPORT.md (state/helper_report.py)."""

from fleet.state.helper_report import (
    HELPER_REPORT_MD,
    REPORT_MAX_BYTES,
    read_report,
    report_path,
...
def test_report_in_artifacts_is_found(tmp_path):
    path = tmp_path / "artifacts" / HELPER_REPORT_MD
...
def test_report_in_outputs_is_found(tmp_path):
    path = tmp_path / OUTPUTS_DIR / HELPER_REPORT_MD
    path.parent.mkdir(parents=True)
    path.write_text("## Root cause\n\nBy hand.\n", encoding="utf-8")
    assert report_path(tmp_path) == path
...
def test_artifacts_wins_when_both_exist(tmp_path):
    artifacts = tmp_path / "artifacts" / HELPER_REPORT_MD
    artifacts.parent.mkdir(parents=True)
    artifacts.write_text("from artifacts", encoding="utf-8")
    outputs = tmp_path / OUTPUTS_DIR / HELPER_REPORT_MD
