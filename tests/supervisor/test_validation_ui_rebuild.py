...
class TestUiRebuildOnUiChange:
    def test_make_ui_build_invoked_when_ui_file_in_diff(
        ...
        monkeypatch.setattr(
            "fleet.supervisor.subprocess.run", _make_subprocess_side_effect(ui_files)
        )
        monkeypatch.setattr(
    ...
    def test_make_ui_build_not_invoked_when_only_backend_files_changed(
        ...
        monkeypatch.setattr(
            "fleet.supervisor.subprocess.run",
...
class TestUiRebuildLogging:
    ...
    def test_ui_rebuild_success_with_mixed_paths_includes_ui(
        ...
        monkeypatch.setattr(
            "fleet.supervisor.subprocess.run", _make_subprocess_side_effect(mixed_files)
        )
        monkeypatch.setattr(
            "asyncio.create_subprocess_exec", fake_create_subprocess_exec
        )
