...
from fleet.workflows.builders.summary_verify import MIN_BYTES, verify_paper_dir
...
def test_complete_entry_passes(tmp_path: Path) -> None:
    ...
    assert verify_paper_dir(_good_entry(tmp_path)) == []
...
def test_source_without_provenance_fails(tmp_path: Path) -> None:
    ...
    assert any("Source:" in failure for failure in verify_paper_dir(entry))
    (entry / "source" / "source.md").unlink()
    assert any("source/source.md" in failure for failure in verify_paper_dir(entry))
...
def test_empty_wiki_fails(tmp_path: Path) -> None:
    ...
    for page in (entry / "wiki").glob("*.md"):
        page.unlink()
    assert any("wiki/" in failure for failure in verify_paper_dir(entry))
