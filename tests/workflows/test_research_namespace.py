...
`knowledge/research/` holds summary_get ENTRY folders (`<PascalSlug>/` with
...
from fleet.workflows.builders import summary_get
...
def test_plan_never_reuses_epic_hub() -> None:
    ...
    assert "Epic-hub rule" in summary_get._PLAN_DESC
    assert "NEVER" in summary_get._PLAN_DESC
    assert "type: Research" in summary_get._PLAN_DESC
